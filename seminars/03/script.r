...
#install.packages(c("corrplot", "ggfortify", "dplyr"))
library(corrplot)
library(ggfortify)
library(dplyr)
...
#long - lots og rows, few columns
#short - fewer rows, more columns

?reshape
#idvar = by which value to resshape
#timevar = column values
#direction = size
data.short <- reshape(data, idvar = "GEO", timevar = "NACE_R2_LABEL", direction = "wide") #short = wide
...
img <- readJPEG("images/001.jpg", native = FALSE)
...
d <- data.pca$x %*% t(data.pca$rotation) +
  matrix(rep(data.pca$center, each = 258), ncol = 186) # FILL
#center - subtract mean (center) of a column from every variable of the column (for every column)
head(d)
...
d <- (data.pca$x %*% t(data.pca$rotation)) *
  matrix(rep(data.pca$scale, each = 258), ncol = 186) + #scale back
  matrix(rep(data.pca$center, each = 258), ncol = 186)  #add means back
